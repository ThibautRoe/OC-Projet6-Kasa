export default function Collapse({ className, title, contentString, contentTable }) {
    ...
    return (
        <>
            {title && (
                <details className={`collapse collapse--${className}`}>
                    <summary className={`collapse__title collapse__title--${className}`}>{title}
                    ...
                    <>
                        {contentString && <p className={`collapse__content collapse__content--${className}`}>{contentString}</p>}
                        {contentTable && contentTable.map((content, index) => (
                            <p key={`content-${index}`} className={`collapse__content collapse__content--${className}`}>{content}</p>
                        ))}
                    </>
                </details>
            )}
